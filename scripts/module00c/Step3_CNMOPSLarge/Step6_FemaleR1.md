...
GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token` \
  tabix -h /.../call-FemaleR1/inputs/1843457483/ref_panel_v1.bincov.bed.gz "chrX" |
 sed 's/Start/start/' | sed 's/Chr/chr/' | sed 's/End/end/' > bincov_chrX.bed

if [ 2 == "normal" ]; then
  mv bincov_chrX.bed bincov_chrX_2.bed
else
  awk -v sex="2" '$5==sex' /.../call-FemaleR1/inputs/-613506260/hg38_v0_sv-resource
...
set +e
bash /opt/WGD/bin/cnMOPS_workflow.sh -S /.../call-FemaleR1/inputs/-1454618493/hg38_
v0_sv-resources_resources_v1_GRCh38_Nmask.bed -x /.../call-FemaleR1/inputs/-1454618
493/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -r 1000 -o . -M bincov_chrX_2.bed &> cnmops.out
RC=$?
set -e
if [ ! $RC -eq 0 ]; then
