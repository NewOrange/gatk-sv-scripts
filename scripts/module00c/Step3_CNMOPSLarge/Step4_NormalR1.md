...
GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token` \
  tabix -h /.../call-NormalR1/shard-21/inputs/1843457483/ref_panel_v1.bincov.bed.gz
 "chr22" | sed 's/Start/start/' | sed 's/Chr/chr/' | sed 's/End/end/' > bincov_chr22.bed

if [ normal == "normal" ]; then
  mv bincov_chr22.bed bincov_chr22_normal.bed
else
  awk -v sex="normal" '$5==sex' /.../call-NormalR1/shard-21/inputs/-613506260/hg38_
v0_sv-resources_ref-panel_1KG_v1_ped_1kg_ref_panel_v1.ped | cut -f2 > ids.to.include
...
set +e
bash /opt/WGD/bin/cnMOPS_workflow.sh -S /.../call-NormalR1/shard-21/inputs/-1454618
493/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -x /.../call-NormalR1/shard-
21/inputs/-1454618493/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -r 1000 -o . -M bincov_chr22_normal.bed &> cnmops.out
RC=$?
set -e
if [ ! $RC -eq 0 ]; then
...
HG00096
HG00097Error: runcnMOPS.R: Less than three samples retained in coverage matrix after filtering on median coverage
Execution halted
```