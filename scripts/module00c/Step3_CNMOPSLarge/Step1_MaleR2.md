...
GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token` \
  tabix -h /.../call-MaleR2/shard-0/inputs/1843457483/ref_panel_v1.bincov.bed.gz "c
hrX" | sed 's/Start/start/' | sed 's/Chr/chr/' | sed 's/End/end/' > bincov_chrX.bed

if [ 1 == "normal" ]; then
  mv bincov_chrX.bed bincov_chrX_1.bed
else
  awk -v sex="1" '$5==sex' /.../call-MaleR2/shard-0/inputs/-613506260/hg38_v0_sv-re
...
set +e
bash /opt/WGD/bin/cnMOPS_workflow.sh -S /.../call-MaleR2/shard-0/inputs/-1454618493
/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -x /.../call-MaleR2/shard-0/inp
uts/-1454618493/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -r 100 -o . -M bincov_chrX_1.bed &> cnmops.out
RC=$?
set -e
if [ ! $RC -eq 0 ]; then
...
set -euo pipefail
GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token` \
  tabix -h /.../call-MaleR2/shard-1/inputs/1843457483/ref_panel_v1.bincov.bed.gz "c
hrY" | sed 's/Start/start/' | sed 's/Chr/chr/' | sed 's/End/end/' > bincov_chrY.bed

if [ 1 == "normal" ]; then
  mv bincov_chrY.bed bincov_chrY_1.bed
else
  awk -v sex="1" '$5==sex' /.../call-MaleR2/shard-1/inputs/-613506260/hg38_v0_sv-re
sources_ref-panel_1KG_v1_ped_1kg_ref_panel_v1.ped | cut -f2 > ids.to.include
...
EMPTY_OUTPUT_ERROR="No CNV regions in result object. Rerun cn.mops with different parameters!"
set +e
bash /opt/WGD/bin/cnMOPS_workflow.sh -S /.../call-MaleR2/shard-1/inputs/-1454618493
/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -x /.../call-MaleR2/shard-1/inp
uts/-1454618493/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -r 100 -o . -M bincov_chrY_1.bed &> cnmops.out
RC=$?
set -e
if [ ! $RC -eq 0 ]; then
  if grep -q "$EMPTY_OUTPUT_ERROR" "cnmops.out"; then
