...
GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token` \
  tabix -h /.../call-MaleR1/shard-0/inputs/1843457483/ref_panel_v1.bincov.bed.gz "chrX"
| sed 's/Start/start/' | sed 's/Chr/chr/' | sed 's/End/end/' > bincov_chrX.bed

if [ 1 == "normal" ]; then
  mv bincov_chrX.bed bincov_chrX_1.bed
else
  awk -v sex="1" '$5==sex' /.../call-MaleR1/shard-0/inputs/-613506260/hg38_v0_sv-resourc
...
set +e
bash /opt/WGD/bin/cnMOPS_workflow.sh -S /.../call-MaleR1/shard-0/inputs/-1454618493/hg38
_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -x /.../call-MaleR1/shard-0/inputs/-14546
18493/hg38_v0_sv-resources_resources_v1_GRCh38_Nmask.bed -r 3 -o . -M bincov_chrX_1.bed &> cnmops.out
RC=$?
set -e
if [ ! $RC -eq 0 ]; then
